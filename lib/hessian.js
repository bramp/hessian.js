...
function read_object(buf, c) {
    ...
    switch (c) {
      ...
      default:
        assert(false, 'Unhandled object type ' + c);
    }  
}
...
function parse_one(buf) {
  ...
  switch (c) {
    ...
    default:
      // Non-standard, but sometimes we just encode data, not a method call
...
exports.parse = function(buf, callback) {
  ...
  while (!buf.eof()) {
    callback( parse_one(buf), buf.offset );
  }
}
